...
def train_xgboost(xgb_config):
    """
    Train XGBoost model with given configuration
    
    Args:
        xgb_config: Dictionary containing model configuration
    
    Returns:
        dict: Training results
    """
    config_name = xgb_config['name']
    artifact_dir = Config.get_model_artifact_dir('xgboost', config_name)
    log_file = os.path.join(artifact_dir, 'training_log.txt')
    
    log_message("="*70, log_file)
    log_message(f"TRAINING XGBOOST: {config_name}", log_file)
    log_message("="*70, log_file)
    ...
    log_message(f"Max features: {xgb_config['tfidf_max_features']}", log_file)
    log_message(f"N-gram range: {xgb_config['tfidf_ngram_range']}", log_file)
    
    vectorizer = TfidfVectorizer(
        max_features=xgb_config['tfidf_max_features'],
        ngram_range=xgb_config['tfidf_ngram_range'],
        min_df=2,
        max_df=0.95,
        sublinear_tf=True
    )
    ...
    log_message("="*70, log_file)
    log_message(f"N estimators: {xgb_config['n_estimators']}", log_file)
    log_message(f"Max depth: {xgb_config['max_depth']}", log_file)
    log_message(f"Learning rate: {xgb_config['learning_rate']}", log_file)
    log_message(f"Subsample: {xgb_config['subsample']}", log_file)
